...
**순서를 지켜야 합니다. 세 단계이며 각각 앞 단계의 완료를 전제합니다.**

1. **백엔드를 먼저 배포하고 엔드포인트 가용을 확인합니다.** BE는 `fd8adfb`로 **머지**됐지만 머지는 배포가 아닙니다. 운영 BE에 `/auth/refresh`·`/auth/logout`이 없는 상태에서 FE를 먼저 배포하면 갱신이 404를 받습니다. 404는 401이 아니므로 `proxy.ts`가 쿠키를 보존하고(순단 보호 로직), 사용자는 로그인 → 15분 뒤 다시 로그인이 반복되며 **자가 복구 경로가 없습니다.**
2. **프론트엔드를 배포합니다.** 이 시점에는 `JWT_EXPIRES_IN`이 아직 `1h`이므로, FE의 15분 쿠키가 먼저 사라져 불필요한 갱신이 한 번 도는 것 외에는 사용자에게 무해합니다.
3. **`JWT_EXPIRES_IN`을 `15m`으로 내립니다.** FE 갱신 로직이 없는 상태에서 먼저 내리면 사용자가 15분 뒤 401을 맞고 복구 경로가 없습니다. env로 조정 가능하므로 코드 변경이 필요 없습니다. (`.env.example`과 `auth.module.ts`의 기본값은 이미 `15m`이며 로컬·신규 환경 기준입니다.)

### 운영 제약 — FE 인스턴스는 하나로 두거나 sticky session을 씁니다

`lib/refresh.ts`의 single-flight 맵은 **프로세스 메모리**입니다. FE 인스턴스가 2개 이상이면 같은 사용자의 병렬 요청이 서로 다른 인스턴스에 떨어져 갱신이 합쳐지지 않습니다.

그때 벌어지는 일은 이렇습니다. 인스턴스 A가 갱신에 성공해 토큰을 회전시키고, 인스턴스 B는 같은 옛 토큰으로 갱신을 시도해 백엔드 CAS 경합에서 패배합니다. 백엔드는 이 경우 세션 가족을 폐기하지 않지만 **반환 코드는 가족 폐기와 동일한 401**입니다(`refresh-tokens.use-case.ts:90-105`, `auth.errors.ts:55-58`). 프론트엔드는 둘을 구분할 수 없어 401을 세션 사망으로 처리하고 쿠키를 지웁니다. 두 응답의 `Set-Cookie`가 브라우저에서 경합해 삭제가 나중에 도착하면 **정상 사용자가 로그아웃됩니다.**

스케일아웃이 필요해지면 `lib/refresh.ts`의 `ponytail:` 주석에 적힌 승격 경로(공유 저장소 락)를 따르거나, 백엔드가 CAS 패배자에게 승자의 토큰 쌍을 돌려주도록 고치면 프론트엔드 락 없이 해소됩니다.
